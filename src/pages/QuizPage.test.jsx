...
describe('QuizPage Component with Regular Data', () => {
  ...
  it('renders the first question correctly', () => {
    ...
    expect(
      screen.getByText(`${firstQuestion.id}. ${firstQuestion.questionText}`)
    ).toBeInTheDocument();
    expect(
      screen.getByText(`Question ${1} of ${mockQuizDataRegular.length}`)
    ).toBeInTheDocument();
  ...
  it('allows selecting an answer and shows feedback (correct answer)', async () => {
    render(<QuizPage />);
    const firstQuestion = mockQuizDataRegular[0];
    const correctAnswerOption = screen.getByRole('button', {
      name: firstQuestion.correctAnswer,
    });
  ...
  it('allows selecting an answer and shows feedback (incorrect answer)', async () => {
    render(<QuizPage />);
    const firstQuestion = mockQuizDataRegular[0];
    const incorrectOptionText = firstQuestion.options.find(
      (opt) => opt !== firstQuestion.correctAnswer
    );
    const incorrectAnswerOption = screen.getByRole('button', {
      name: incorrectOptionText,
    });
    ...
    expect(
      screen.getByText(
        `🤔 Incorrect. The correct answer is: ${firstQuestion.correctAnswer}`
      )
    ).toBeInTheDocument();
  ...
  it('navigates to the next question', async () => {
    ...
    const firstAnswerOption = screen.getByRole('button', {
      name: firstQuestion.options[0],
    });
    await act(async () => {
    ...
    expect(
      screen.getByText(`${secondQuestion.id}. ${secondQuestion.questionText}`)
    ).toBeInTheDocument();
    expect(
      screen.getByText(`Question ${2} of ${mockQuizDataRegular.length}`)
    ).toBeInTheDocument();
    expect(
      screen.getByRole('button', { name: /Next Question/i })
    ).toBeDisabled();
  ...
  it('completes the quiz and shows results', async () => {
    ...
    for (let i = 0; i < mockQuizDataRegular.length; i++) {
      ...
      const nextButtonText =
        i === mockQuizDataRegular.length - 1
          ? /Show Results/i
          : /Next Question/i;
    ...
    expect(
      screen.getByText(
        `You scored ${expectedScore} out of ${mockQuizDataRegular.length}.`
      )
    ).toBeInTheDocument();
    expect(
  ...
  it('displays SQL query if present in question data', () => {
    render(<QuizPage />);
    expect(
      screen.getByText(mockQuizDataRegular[0].sqlQuery)
    ).toBeInTheDocument();
  });
...
describe('QuizPage Component - Empty Quiz Data', () => {
  ...
  it('handles empty quizQuestions.json gracefully', () => {
    ...
    expect(
      screen.getByText(/No quiz questions available./i)
    ).toBeInTheDocument();
  });
});
